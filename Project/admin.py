...
from .models import Usuario, Zona, Representante, Paciente, Ubicacion, Tipo_insumo, Lote, Laboratorio, Producto, Solicitud, DetalleSolicitud, Empleado, Factura, Tipo_mov, Movimiento_inventario
...
class SolicitudAdmin(admin.ModelAdmin):
    list_display = ('id_soli', 'fecha_soli', 'descripcion_soli', 'get_total_cantidad', 'estado')

    def get_total_cantidad(self, obj):
        return sum(detalle.cantidad for detalle in obj.detallesolicitud_set.all())

@admin.register(DetalleSolicitud)
class DetalleSolicitud(admin.ModelAdmin):
