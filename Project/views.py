...
from .forms import FormularioLogin, FormularioUsuario, FormularioUsuarioEdicion, ZonaForm, RepresentanteForm, PacienteForm, UbicacionForm, Tipo_insumoForm, LoteForm, LaboratorioForm, ProductoForm, EstadoSolicitudForm, SolicitudForm, SolicitudEdicionForm, EmpleadoForm, FacturaForm, Tipo_movForm, Movimiento_inventarioForm, LandingContentForm
from .models import Usuario, Zona, Representante, Paciente, Ubicacion, Tipo_insumo, Lote, Laboratorio, Producto, Solicitud, Empleado, Factura, Tipo_mov, Movimiento_inventario, LandingContent
...
def landing(request):
    landing_content = LandingContent.objects.first()  # Obtener el primer objeto LandingContent
    return render(request, 'Landingpage/landing.html', {'landing_content': landing_content})

def edit_landing(request):
    instance = LandingContent.objects.first()
    if request.method == 'POST':
        form = LandingContentForm(request.POST, request.FILES, instance=instance)
        if form.is_valid():
            form.save()
            return redirect('landing')
    else:
        form = LandingContentForm(instance=instance)
    return render(request, 'Landingpage/edit_landing.html', {'form': form})
...
class EstadoSolicitud(UpdateView):
    model = Solicitud
    form_class = EstadoSolicitudForm
    template_name = 'Solicitud/estadoSolicitud.html'
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if estado == 'APROBADA':
                if (usuario_actual.cargo in ['ATENCION AL CLIENTE', 'ADMINISTRADOR'] and
                        (solicitud.aceptaciones == 0 or solicitud.aceptaciones == 2)):
                    solicitud.aceptaciones += 1
                    solicitud.actualizar_estado()
                    mensaje = f'La solicitud ha sido aceptada ({solicitud.aceptaciones}/3 aceptaciones)'
                    return JsonResponse({'mensaje': mensaje}, status=201)
                elif usuario_actual.cargo == 'ALMACENISTA' and solicitud.aceptaciones == 1:
                    solicitud.aceptaciones += 1
                    solicitud.actualizar_estado()
                    mensaje = f'La solicitud ha sido aceptada por el ALMACENISTA ({solicitud.aceptaciones}/3 aceptaciones)'
                    return JsonResponse({'mensaje': mensaje}, status=201)
                else:
                    mensaje = 'No tienes permiso para realizar esta acción.'
                    return JsonResponse({'mensaje': mensaje}, status=403)
            ...
            mensaje = 'Debes seleccionar un estado para la solicitud.'
            return JsonResponse({'mensaje': mensaje}, status=400)
