...
from .models import Usuario, Zona, Representante, Paciente, Ubicacion, Tipo_insumo, Lote, Laboratorio, Producto, Solicitud, DetalleSolicitud, Empleado, Factura, Tipo_mov, Movimiento_inventario, LandingContent


class LandingContentForm(forms.ModelForm):
    class Meta:
        model = LandingContent
        fields = ['imagen1', 'imagen2', 'imagen3', 'imagen4', 'imagen5', 'texto1']
        labels = {
            'imagen1': 'Imagen 1 de Carrusel',
            'imagen2': 'Imagen 2 de Carrusel',
            'imagen3': 'Imagen 3 de Carrusel',
            'imagen4': 'Imagen 4 de Carrusel',
            'imagen5': 'Imagen 5 de Carrusel',
            'texto1': 'Texto del Cintillo'
        }
...
class SolicitudForm(forms.ModelForm):
	...
	class Meta:
		model = Solicitud
		fields = ('cod_pac', 'descripcion_soli', 'id_producto', 'cantidad', 'recipe', 'estado')
		label = {
			'cod_pac': 'Paciente',
		...
		widgets ={
			...
			'estado': forms.Select(
				choices=[('','SELECCIONA UNA OPCIÓN'), ('EN PROCESO', 'EN PROCESO'), ('ACEPTADA', 'ACEPTADA'), ('RECHAZADA', 'RECHAZADA')],
				attrs = {
					'class': 'form-control',
				}
			),
	...
	def save(self, commit=True):
		...
		if commit:
			...
			for producto in self.cleaned_data['id_producto']:
				cantidad = self.cleaned_data['cantidad']
				DetalleSolicitud.objects.create(solicitud=solicitud, producto=producto, cantidad=cantidad)
			print("Detalles de solicitud guardados exitosamente.")
		return solicitud
...
class SolicitudEdicionForm(forms.ModelForm):
	...
	class Meta:
		model = Solicitud
		fields = ('descripcion_soli', 'id_producto', 'recipe')
		label = {
			'descripcion_soli': 'Descripción de la Solicitud',
			'id_producto': 'Producto',
			'recipe':'Recipe',
...
class EstadoSolicitudForm(forms.ModelForm):
    estado = forms.ChoiceField(
        ...
        widget=forms.RadioSelect(attrs={'class': 'btn-check', 'autocomplete': 'off', 'required': True})
    )
