	...
	$.ajax({
		...
		success: function(response){
			...
			for(let i = 0;i < response.length;i++){
				...
				fila += '<td><a class="nav-link w-50 text-lg" title="Estado de la Solicitud" aria-label="Estado de la Solicitud" onclick="abrir_modal_edicionSolicitud(\'/estadoSolicitud/' + response[i]["pk"] + '/\');"><i class="fa fa-envelope-open fa-2x"></i></a>';
				fila += '<a class="nav-link w-50 text-lg" title="Editar Solicitud" aria-label="Editar Solicitud" onclick="abrir_modal_edicionSolicitud(\'/editarSolicitud/' + response[i]["pk"] + '/\');"><i class="fa fa-pencil-square-o fa-2x"></i></a>';
				fila += '<a class="nav-link w-50 text-lg" title="Eliminar Solicitud" aria-label="Eliminar Solicitud" onclick="abrir_modal_eliminacionSolicitud(\'/eliminarSolicitud/' + response[i]["pk"] + '/\');"><i class="fa fa-times fa-2x"></i></a></td>';
				fila += '</tr>';
...
function registrarSolicitud() {
    ...
    // Obtener los valores de los productos y cantidades y agregarlos al FormData
    $('#form_creacionSolicitud .producto').each(function(index, element) {
        formData.append('productos[]', $(element).val());
    });
    $('#form_creacionSolicitud .cantidad').each(function(index, element) {
        formData.append('cantidades[]', $(element).val());
    });
...
function registrarSolicitud() {
    ...
    // Obtener los valores de los productos y cantidades y agregarlos al FormData
    formData.append('cantidad', $('#form_creacionSolicitud input[name^="cantidad"]').map(function(){
        return $(this).val();
    }).get());
